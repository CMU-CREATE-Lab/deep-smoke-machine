...
class I3dLearner(BaseLearner):
    def __init__(self,
            batch_size_train=16, # size for each batch for training (8 max for each GTX 1080Ti)
            batch_size_test=64, # size for each batch for testing (32 max for each GTX 1080Ti)
            batch_size_extract_features=16, # size for each batch for extracting features
    ...
    def test(self,
            p_model=None, # the path to load the pretrained or previously self-trained model
            save_tensorboard_path="../data/saved_i3d/[model_id]/run/", # path to save data ([model_id] will be replaced)
            save_log_path="/var/www/smoke-detection/deep-smoke-machine/back-end/data/saved_i3d/[model_id]/log/test.log" # path to save log files ([model_id] will be replaced)
            ):
        # Check
        if p_model is None:
        ...
        with torch.no_grad():
            ...
            for d in dataloader["test"]:
                ...
                labels = self.to_variable(labels)
                pred = self.make_pred(model, frames)
                p = pred.cpu().detach().numpy().max(axis=2)
                for i in range(p.shape[0]):
                    all_pred[counter][i] = (str(self.mode + ":"), p[i].tolist(), "correct:", l[i], "name:", names[i])
                pred_labels += self.labels_to_list(pred.cpu().detach())

        with open('pred_i3d_rgb.json', 'w') as f:
