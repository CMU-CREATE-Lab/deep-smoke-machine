    ...
    def test(self,
        ...
        with torch.no_grad():
            for d in dataloader["test"]:
                ...
                pred = torch.zeros_like(pred_rgb)
                all_pred[counter] = {}
                for i in range(pred_rgb.shape[0]):
                    all_pred[counter][i] = ("rgb:", pred_rgb[i].cpu().detach().tolist(), "flow:", pred_flow[i].cpu().detach().tolist(), "correct:", l[i], "name:", file_names[i])
                    pred[i][0] = (pred_rgb[i][0] + pred_flow[i][0])/2
                    pred[i][1] = (pred_rgb[i][1] + pred_flow[i][1])/2
                #pred[0] = (pred_rgb[0] + pred_flow[0])/2
                #pred[1] = (pred_rgb[1] + pred_flow[1])/2
                _, predicted = torch.max(pred, dim=1)
